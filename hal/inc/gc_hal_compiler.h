...
{
    gctSIZE_T     blockSize;
    gctSIZE_T     elementAlignment;
    gctSIZE_T     numOfBlocks;
    gcsBlockNode  blocks;
...
struct _gcsHINT
{
    ...
    /* the vsOutput0 is for position output, the VS output 17
     * is the varying16*/
    gctUINT     vsOutput17;
};
...
typedef enum _gceSHADER_FLAGS
{
    ...
    */
    ...
    /* 5_0_3_gc1000 and 5_1_0_gc880 have HW bug1032.
       To workaround hardware bug(1032) which flood the side band fifo
       (256 items) and caused gpu hang when more texld is send to
       the fifo, we need to make sure not to spawn too many threads
       by increase the max register number to proper value;
       return true if the maxTemp is adjusted
    */
    gcvSHADER_TEXLD_WORKAROUND = 0x400,
...
typedef struct _gcOPTIMIZER_OPTION
{
    ...
    /* debug & dump options:
         ...
         VC_OPTION=-DUMP:SRC:OPT|:OPTV|:CG|:CGV|:ALL|:ALLV|:UNIFORM
...
typedef enum _gceUNIFORM_FLAGS
{
    ...
    gcvUNIFORM_CONST                       = 0x800000,
    gcvUNIFORM_HW_SAMPLER                  = 0x1000000,
    gcvUNIFORM_TEX_RCPOFWH                 = 0x2000000,
}
gceUNIFORM_FLAGS;

#define isUniformUsedInShader(uniform) ((uniform)->flags & gcvUNIFORM_USED_IN_SHADER)
#define isUniformUsedInLTC(uniform)    ((uniform)->flags & gcvUNIFORM_USED_IN_LTC)
...
/*******************************************************************************
**  gcSHADER_SetUniformInitializer
**
**  Add an initializer to a uniform.
**
**  INPUT:
**
**      gcsValue * InitValue
**          the initializer value for the Uniform, the value
**          is scalar or vector (at most vec4) value
**
**  OUTPUT:
**
**      gcUNIFORM * Uniform
...
*/
gceSTATUS
gcUNIFORM_SetInitializer(
    IN OUT gcUNIFORM   Uniform,
    IN gcsValue *      InitValue
    );

/*******************************************************************************
**  gcUNIFORM_UploadData
**
**  Upload the data of a uniform to HW.
**
**  INPUT:
**
**      gcUNIFORM Uniform
...
**          array.
**
**      const gctCONST_POINTER Data
**          Pointer to a buffer holding the data for the uniform.
**
**      gctBOOL ConvertToFloat
**          Whether the integer data need to be converted to float before uploading.
**
**  OUTPUT:
**
**      Nothing.
...
/*******************************************************************************
**  gcUNIFORM_BLOCK_Bind
**
**  Bind the uniform block with buffer object.
**
**  INPUT:
**
**      gcSHADER Shader
...
**
**      gcUNIFORM Uniform
**          Pointer to entry gcUNIFORM object of that uniform block.
**
**      gcoBUFOBJ BufObj
**          Pointer to a gcoBUFOBJ the block bound to
**          array.
**
**      gctSIZE_T Offset
**          The offset to that gcoBUFOBJ.
**
**      gctSIZE_T Size
**          Size will bind.
**
...
gceSTATUS
gcSHADER_GetTransformFeedbackBufferUniform(
    IN gcSHADER Shader,
    IN gctSIZE_T Count,
    OUT gceFEEDBACK_BUFFER_MODE *BufferMode,
    OUT gcUNIFORM * Uniform
    );

gceSTATUS
gcSHADER_GetTransformFeedbackStateUniform(
    IN gcSHADER Shader,
    OUT gcUNIFORM * Uniform
    );
...
typedef struct _gcsTexturePatch
{
    gctBOOL                 isShadowType;
    gceTEXTURE_COMPARE_MODE compareMode;
    gceCOMPARE              compareFunc;
} gcsTexturePatch;

gceSTATUS
gcProgram_PatchShaderTexture(
    IN gcoHARDWARE Hardware,
    IN gctUINT32_PTR StateBuffer,
    IN gctSIZE_T StateBufferSize,
    IN gcsTexturePatch *Patches
    );
