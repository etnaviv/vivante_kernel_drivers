    ...
    )
{
    ...
    gcmkONERROR(
        gckVIDMEM_NODE_Allocate(Kernel, node, &handle));
...
#if !MRVL_DISABLE_USER_SIGNAL_TIMEOUT
...
#if gcdMULTI_GPU
        Interface->u.ChipInfo.coreCount = Kernel->hardware->identity.gpuCoreCount;
#endif
        break;
